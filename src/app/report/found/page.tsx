...
const FOUND_CATEGORIES = ["Wallet", "ID Card", "Phone", "Keys", "Bag", "Laptop", "Other"] as const;
...
export default function ReportFoundPage() {
  ...
  const [category, setCategory] = useState<string>(FOUND_CATEGORIES[0]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !locationFound.trim()) {
    ...
    try {
      ...
      if (insertError || !insertedItem?.id) {
        toast.error(insertError?.message || "Unable to submit found item report.");
        return;
      }

      if (photoFile) {
        const extension = getFileExtension(photoFile);
        const filePath = `${authData.user.id}/found/${insertedItem.id}/${Date.now()}.${extension}`;
  ...
  const handleReset = () => {
    ...
    setTitle("");
    setDescription("");
    setCategory(FOUND_CATEGORIES[0]);
    setLocationFound("");
    setPhotoFile(null);
    setPreviewUrl("");
  };
      ...
      <main className="w-full md:max-w-md lg:max-w-lg mx-auto px-4 pt-20 pb-4">
        <form onSubmit={handleSubmit} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="foundTitle" className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Item Name</Label>
            <Input
              id="foundTitle"
              value={title}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="foundCategory" className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Category</Label>
            <select
              id="foundCategory"
              value={category}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="foundLocation" className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Location Found</Label>
            <Input
              id="foundLocation"
              value={locationFound}
              onChange={(e) => setLocationFound(e.target.value)}
              className="h-12 rounded-xl bg-gray-50/50 border-gray-200 focus-visible:ring-[#203e7e]"
              placeholder="Example: Library cafe"
            />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="foundDescription" className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Description</Label>
            <textarea
              id="foundDescription"
              value={description}
              ...
              placeholder="Describe the item details so owner can identify it."
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="foundCamera" className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Item Photo (Camera Only)</Label>
